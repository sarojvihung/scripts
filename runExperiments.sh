...
declare -a ueNodes=("10.10.1.14" "10.10.1.16")
...
do
    ...
    do
        ...
        #start-ran
        bash /opt/scripts/runNodeCmd.sh "nr-gnb -c /opt/UERANSIM/config/open5gs-gnb.yaml > /dev/null 2>&1 &" 12 14

        #bash /opt/scripts/runNodeCmd.sh "/opt/scripts/launchUeSim.py > /dev/null 2>&1 &" 13 15
        ...
        #stop-ran
        bash /opt/scripts/runNodeCmd.sh "pkill -f nr-ue" 13 15

        sleep 5

        bash /opt/scripts/runNodeCmd.sh "pkill -f nr-gnb" 12 14
