...
if [ "$#" -eq 3 ]; then
  IP="$2"
  PORT="$3"
  framenum=$(tshark -r /tmp/ui.pcapng -Y 'http && http.request.uri=="/api/db/Subscriber"' | sed -n '1p' | awk '{print $1}')
  cookie=$(tshark -r /tmp/ui.pcapng -Y frame.number==$framenum -T json | grep '"http.cookie": "connect.sid=' | awk -v FS='(sid=|",)' '{print $2}')
  auth=$(tshark -r /tmp/ui.pcapng -Y frame.number==$framenum -T json | grep "Authorization: Bearer" | awk -v FS='(Bearer |\r\n",)' '{print $2}' | sed 's/.\{6\}$//')
  xcrf=$(tshark -r /tmp/ui.pcapng -Y frame.number==$framenum -T pdml | grep "X-CSRF-TOKEN:" | awk -v FS='(X-CSRF-TOKEN: |\r\n",)' '{print $2}' | sed 's/\s.*$//' | sed 's/.\{5\}$//')
else 
