...
declare -a master_node_cmds=("$k8s_create_cmd" "export KUBECONFIG=/etc/kubernetes/admin.conf" "KUBECONFIG=/etc/kubernetes/admin.conf && kubectl apply -f /opt/k8s/kube-flannel.yml" "sleep 60" "KUBECONFIG=/etc/kubernetes/admin.conf && kubectl get node -owide" "systemctl restart containerd" "sleep 10" "KUBECONFIG=/etc/kubernetes/admin.conf && kubectl apply -f /opt/k8s/metrics-server.yaml" "KUBECONFIG=/etc/kubernetes/admin.conf && kubectl get pods -A")
